...
*生成与证据说明：ImageGen 合成概念底图 + Pillow 确定性证据标注；非现状或审批证据。* [source:MEDIA-LH-CORRIDOR-AERIAL-V1]
...
*生成与证据说明：ImageGen 合成概念底图 + Pillow 确定性证据标注；非现状或审批证据。* [source:MEDIA-LH-AXONOMETRIC-V1]
...
*生成与证据说明：ImageGen 合成概念底图 + Pillow 确定性证据标注；非现状或审批证据。* [source:MEDIA-LH-RESPONSIBILITY-ROUTE-V1]
...
*生成与证据说明：三张 ImageGen 合成概念底图 + Pillow 确定性证据标注；非现状或审批证据。* [source:MEDIA-LH-CAPABILITY-LAB-V1] [source:MEDIA-LH-HUMAN-INTERCHANGE-V1] [source:MEDIA-LH-REPAIR-WORKSHOP-V1]
...
![十二个中性动作镜头与独立场景合同清单；两者不作一一对应，图像不等于场景验证](assets/figures/scenario-matrix.png)

*生成与证据说明：ImageGen 合成拼贴与合同不作一一对应；Pillow 后叠动作和合同文字，图像不等于场景验证。* [source:MEDIA-LH-SCENARIO-MOSAIC-V1]
...
*生成与证据说明：ImageGen 合成概念底图 + Pillow 确定性证据标注；非现状或绩效证据。* [source:MEDIA-LH-CORRIDOR-AERIAL-V1]
...
本方案正文、结构化数据、代码式几何、版式与确定性标注由 Codex × AmyLili28 为本次投稿生成；七张无文字概念底图由 OpenAI ImageGen 通过 Codex 内置图像生成服务完成：六张为纯文字生成，`lh-human-interchange-v1.jpg` 是对同一会话中先前由 ImageGen 文字生成的草稿作二次编辑，只把入口改成连续无台阶界面；客户端未暴露精确模型标识，七张图均在 `sources.json` 以 `MEDIA-LH-*` 逐项登记。AmyLili28 确定“搭把手”的方向、人情味、独立空间图与无假文字要求；Codex 将获准方案概念、任务书和临时几何的描述转写为提示词，筛选或编辑底图，再用 Pillow 后叠可核对的标题、编号、指标、临时几何索引与证据边界。生成流程未上传或使用场地照片、网页图片、地图瓦片、可识别真人、商标、同行图面或艺术家风格；唯一图像输入是上述同一会话的自生成草稿。人物与地点均为合成情境。根据本包引用的 OpenAI 公开条款，OpenAI 与用户之间的输出权利在法律允许范围内归用户或由 OpenAI 转让，但输出可能不唯一，输入、输出、第三方权利与具体用途仍由提交者核查 [source:OPENAI-TERMS-OF-USE-2026-08-31] [source:OPENAI-SERVICE-TERMS-2026-08-31]。每张底图的提示词摘要、人工指导、输入、转换、合成状态、条款依据与限制见 `sources.json` 和 `report/copyright_statement.md`；`COMMUNITY-DISPLAY-ONLY` 不保证可版权性、独占性、无侵权或后续商用许可。

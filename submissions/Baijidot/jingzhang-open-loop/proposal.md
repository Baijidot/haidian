---
title: "京张开源环：可读、可问、可退出的贡献驱动型AI城市公共创新回路"
...
summary: "以遗址公园为公共主轴，以三处重点区为创新锚点，把开源贡献、城市测试、日常服务与长期运营闭合为可读、可问、可退出的公共创新回路。"
...
开源环的公共承诺是：**可读**（提供中文、英文、图形、语音/易读等适配信息，并说明AI建议的适用边界）、**可问**（可追问来源、责任人、置信范围与纠错进度）、**可退出**（可拒绝识别、撤回同意、转人工或停止试点）。三项权利必须落到入口标识、服务界面、工单和运营排班，不以“已阅读条款”替代真实选择。
...
每个AI触点统一展示六项公共接口：**这是什么、为谁服务、依据什么、谁来复核、如何纠错、如何退出**。现场至少保留高对比文字/图形、可触摸或语音入口、无智能手机路径和人工联系方式；关键服务在人工不可用时不得伪装为“全自动可办”。无障碍使用者、银龄群体和一线服务人员共同参与上线验收，问题未关闭不得扩大试点 [standard:BARRIER-FREE-ENVIRONMENT-LAW]。

每条模型回答、文化叙事、政策匹配和运营提示附“来源护照”：来源ID与版本、用途与适用范围、生成/更新时间、许可或授权状态、已知限制、人工责任人、纠错入口和停止展示条件。护照回接 `sources.json`，原始证据与机器解释分栏呈现；来源不明、授权不清或争议未决的内容暂停公开，不由流畅表达掩盖证据缺口 [source:SOURCE-REGISTRY]。
...
| # | 场景 / 位置 | 服务对象与闭环 | 数据、人工复核与退出 |
| --- | --- | --- | --- |
| 01 | 开源发布厅 / 原点 | 项目发布→社区评议→版本迭代 | 自愿提交；主持人审核；可撤回署名 |
| 02 | 模型安全沙盒 / 众智园 | 红队测试→问题分级→修复复测 | 隔离测试；安全负责人签字；严重事件即停 |
| 03 | 端侧算力驿站 / 三核 | 预约算力→能耗计量→错峰激励 | 账户最小化；运维复核；可取消预约与删号 |
| 04 | 可解释慢行导航 / 公园轴 | 断点识别→绕行建议→工单反馈 | 匿名流量；交通人员复核；保留静态导视 |
| 05 | 无障碍路径助手 / 全环 | 障碍上报→安全路径→整改回访 | 不保存健康标签；人工确认；可转人工引导 |
| 06 | 清河雨洪预警庭 / 众智园 | 水位感知→分级提示→封控解除 | 环境传感；防汛值守决策；人工解除封控 |
| 07 | 企业服务共驾 / 三核 | 需求诊断→政策匹配→专员办理 | 企业授权；专员终审；可拒绝推荐 |
| 08 | 社区AI帮办 / 社区节点 | 事项解释→材料清单→人工窗口 | 不代办审批；柜员确认；无手机可办 |
| 09 | 多语遗产导览 / 公园轴 | 故事检索→路线讲解→纠错回写 | 来源护照；专家校订；争议内容暂停 |
| 10 | 低碳活动调度 / 全环 | 人流预测→场地排期→能源复盘 | 聚合数据；运营经理确认；可人工改排 |
| 11 | 智能终端试用街 / 大钟寺 | 体验→无障碍测试→企业改进 | 明示同意；观察员记录；可随时停止 |
| 12 | 夜间安全共治 / 原点 | 照明巡检→风险上报→物业闭环 | 不做人脸追踪；人工派单；保留电话报修 |
...
每个试点在启动前登记期限、样本范围、数据上限、值班责任人、人工替代和拆除/迁移方法；出现人身安全事件、越权采集、持续歧视性误判、人工通道中断或连续两个评估周期未达公共价值下限时立即暂停。暂停后先保障人工服务和现场安全，再删除或封存数据、公开事件摘要并由理事会决定修复复开或退出。退出不等于失败，而是开源环把风险控制和公共选择纳入迭代的正常机制。
...
运营绩效在真实试点建立基线后填值，不以客流或自动化率替代公共价值：

| 运营指标 | 定义与记录方式 | 继续/调整/退出用途 |
| --- | --- | --- |
| 易读信息覆盖率 | 通过易读审校的关键触点 / 全部关键触点 | 低于门槛不得扩面 |
| 无手机服务完成率 | 不依赖智能手机完成的有效服务 / 抽样任务 | 识别数字排斥并补人工能力 |
| 人工接管可用率 | 营业时段内可在承诺时间接管的比例 | 连续不达标暂停高风险场景 |
| 来源护照完整率 | 八项护照字段齐全且可追溯的公开输出比例 | 缺失内容下线补证 |
| 问题关闭中位时长 | 从公众上报到验证关闭的中位时间 | 识别维护拥堵与责任空档 |
| 可停止试点达标率 | 完成停机、删数/封存、人工替代演练的试点比例 | 未演练不得转常态运营 |
| 无障碍共测参与率 | 有残障/银龄共同测试的场景 / 应测场景 | 低于门槛退回设计阶段 |
| 公共时段保障率 | 实际免费公共时段 / 承诺免费公共时段 | 防止商业活动挤出公共性 |
